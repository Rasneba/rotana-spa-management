...
export const SPA_MODULES: SpaModuleDefinition[] = [
  ...
  moduleDefinition({
    ...
    fields: [
      { key: "customer_name", label: "Customer Name", type: "text", required: true, list: true },
      { key: "request_type", label: "Request Type", type: "select", required: true, options: ["booking", "reschedule", "facility", "billing", "complaint", "special-assistance", "other"], list: true },
  ...
  moduleDefinition({
    ...
    slug: "services",
    title: "Services",
    singular: "Spa Service",
    description: "Configure the treatment catalogue, duration, pricing and availability.",
    resource: "spa_services",
    icon: "flower2",
    ...
    fields: [
      { key: "service_name", label: "Service Name", type: "text", required: true, list: true },
      { key: "category", label: "Category", type: "select", required: true, options: ["massage", "facial", "body-treatment", "wellness", "beauty", "other"], list: true },
      { key: "duration_minutes", label: "Duration (minutes)", type: "number", required: true, min: 5, step: 5, list: true },
      { key: "price", label: "Price (ETB)", type: "currency", required: true, min: 0, step: 0.01, list: true },
  ...
  moduleDefinition({
    ...
    slug: "packages",
    title: "Packages",
    singular: "Spa Package",
    description: "Bundle services into sellable packages with validity and usage limits.",
    resource: "spa_packages",
    icon: "gift",
    primaryField: "package_name",
    amountField: "price",
    defaultStatus: "active",
    statusOptions: ["draft", "active", "inactive", "expired"],
    fields: [
      { key: "package_name", label: "Package Name", type: "text", required: true, list: true },
      { key: "included_services", label: "Included Services", type: "textarea", required: true },
      { key: "validity_days", label: "Validity (days)", type: "number", required: true, min: 1, step: 1, list: true },
      { key: "price", label: "Price (ETB)", type: "currency", required: true, min: 0, step: 0.01, list: true },
      { key: "max_uses", label: "Maximum Uses", type: "number", min: 1, step: 1 },
      { key: "description", label: "Description", type: "textarea" },
    ],
  }),
  moduleDefinition({
    ...
    slug: "service-usage",
    title: "Service Usage",
    singular: "Service Usage",
    description: "Record delivered treatments, staff assignments, room usage and charges.",
    resource: "spa_service_usage",
    icon: "activity",
    primaryField: "customer_name",
    dateField: "used_at",
    amountField: "charge_amount",
    defaultStatus: "booked",
    statusOptions: ["booked", "in-service", "completed", "cancelled", "no-show"],
    fields: [
      { key: "customer_name", label: "Customer Name", type: "text", required: true, list: true },
      { key: "service", label: "Service", type: "text", required: true, list: true },
      { key: "therapist", label: "Therapist", type: "text", required: true, list: true },
      { key: "room", label: "Treatment Room", type: "text", list: true },
      { key: "used_at", label: "Service Date / Time", type: "datetime-local", required: true },
      { key: "quantity", label: "Quantity", type: "number", min: 1, step: 1 },
      { key: "charge_amount", label: "Charge Amount (ETB)", type: "currency", min: 0, step: 0.01 },
  ...
  moduleDefinition({
    ...
    title: "Products",
    singular: "Product",
    description: "Manage retail products, cost, price and stock thresholds.",
    resource: "inventory_products",
    icon: "box-seam",
    primaryField: "product_name",
    dateField: "expiry_date",
    amountField: "selling_price",
    ...
    fields: [
      ...
      { key: "selling_price", label: "Selling Price (ETB)", type: "currency", min: 0, step: 0.01 },
      { key: "cost_price", label: "Cost Price (ETB)", type: "currency", min: 0, step: 0.01 },
  ...
  moduleDefinition({
    section: "charges",
    slug: "pending-charges",
    title: "Pending Charges",
    singular: "Pending Charge",
    description: "Capture unbilled services and products before cashier settlement.",
    resource: "charges_pending",
    icon: "hourglass-split",
    primaryField: "customer_name",
    dateField: "charge_date",
    amountField: "amount",
    defaultStatus: "pending",
    statusOptions: ["pending", "sent", "paid", "void"],
    fields: [
      { key: "customer_name", label: "Customer Name", type: "text", required: true, list: true },
      { key: "source", label: "Charge Source", type: "select", required: true, options: ["spa", "gym", "membership", "retail", "facility", "other"], list: true },
      { key: "description", label: "Description", type: "text", required: true, list: true },
      { key: "reference", label: "Reference", type: "text", list: true },
      { key: "charge_date", label: "Charge Date", type: "datetime-local", required: true },
      { key: "amount", label: "Amount (ETB)", type: "currency", required: true, min: 0, step: 0.01, list: true },
      { key: "notes", label: "Notes", type: "textarea" },
    ],
  }),
  moduleDefinition({
    section: "charges",
    slug: "send-to-cashier",
    title: "Send To Cashier",
    singular: "Cashier Dispatch",
    description: "Dispatch charge batches to a cashier and follow them through payment.",
    resource: "charges_cashier",
    icon: "send",
    primaryField: "customer_name",
    dateField: "sent_at",
    amountField: "amount",
    defaultStatus: "queued",
    statusOptions: ["queued", "sent", "accepted", "paid", "rejected"],
    fields: [
      { key: "customer_name", label: "Customer Name", type: "text", required: true, list: true },
      { key: "charge_reference", label: "Charge Reference", type: "text", required: true, list: true },
      { key: "cashier", label: "Cashier", type: "text", list: true },
      { key: "sent_at", label: "Sent At", type: "datetime-local", required: true, list: true },
      { key: "amount", label: "Amount (ETB)", type: "currency", required: true, min: 0, step: 0.01, list: true },
      { key: "payment_method", label: "Expected Method", type: "select", options: ["cash", "card", "bank-transfer", "mobile", "other"] },
      { key: "notes", label: "Notes", type: "textarea" },
    ],
  }),
  moduleDefinition({
    section: "charges",
    slug: "external-receipts",
    title: "External Receipt",
    singular: "External Receipt",
    description: "Register and verify receipts paid through external providers.",
    resource: "charges_external_receipts",
    icon: "receipt",
    primaryField: "receipt_number",
    dateField: "receipt_date",
    amountField: "amount",
    defaultStatus: "submitted",
    statusOptions: ["submitted", "verified", "rejected", "reconciled"],
    fields: [
      { key: "receipt_number", label: "Receipt Number", type: "text", required: true, list: true },
      { key: "customer_name", label: "Customer Name", type: "text", required: true, list: true },
      { key: "provider", label: "Provider", type: "text", required: true, list: true },
      { key: "receipt_date", label: "Receipt Date", type: "date", required: true, list: true },
      { key: "amount", label: "Amount (ETB)", type: "currency", required: true, min: 0, step: 0.01, list: true },
      { key: "attachment_url", label: "Receipt Attachment URL", type: "url" },
      { key: "notes", label: "Notes", type: "textarea" },
    ],
  }),
  moduleDefinition({
...
export const SPA_REPORTS: SpaReportDefinition[] = [
  ...
  { key: "revenue", slug: "revenue", title: "Revenue Report", description: "Revenue totals and payment trends by period and method.", resource: "reports_revenue", icon: "cash-stack" },
  { key: "therapist", slug: "therapist", title: "Therapist Report", description: "Therapist service volume, utilization and generated charges.", resource: "reports_therapist", icon: "person-heart" },
